/* simulateMS.c - A MEX-file for simulating a mismatch-shaping DAC
 * %[sv,sx,sigma_se,max_sx,max_sy] = simulateMS(v,M=16,mtf,d=0,dw=[1..],sx0[0..])
 * %Simulate a mismatch-shaping element DAC.
 * %Inputs:
 * % v is a vector of the digital input values.
 * % mtf is the mismatch transfer funtion, given in zero-pole form.
 * % M is the number of unit elements.
 * % d Dither uniformly-distributed in [-d d] is added to the vector quantizer
...
void processInputArgs( int nrhs, const mxArray *prhs[] ){
    ...
    if( nrhs < 1 )
        fatalError("At least one input argument is needed.");
    pv = mxGetPr(prhs[0]);
    N = mxGetN(prhs[0]) > mxGetM(prhs[0])? mxGetN(prhs[0]):mxGetM(prhs[0]);
    
    if( nrhs > 1 && mxGetM(prhs[1])>0 && !mxIsNaN(*mxGetPr(prhs[1])) )
        M = (int)(*mxGetPr(prhs[1]));
    else
        M = 16;

    if( nrhs <=2 || mxGetM(prhs[2])==0 || mxIsNaN(*mxGetPr(prhs[2])) ){
        zeros = mxCreateDoubleMatrix(1,1,mxREAL);
        *mxGetPr(zeros) = 1;
        poles = mxCreateDoubleMatrix(1,1,mxREAL);
        *mxGetPr(poles) = 0;
        MTF_order = 1;
    }
    /* Determine the form of the mtf */
    else if( mxIsClass(arg2,"zpk") ){		/* NTF in zpk form */
    ...
    if( nrhs > 3 && mxGetM(prhs[3])>0 && !mxIsNaN(*mxGetPr(prhs[3])) ){
        d = *mxGetPr(prhs[3]);
        dither = 1;
    }else{
        d = 0;
    ...
    initialize_dw(nrhs>4?prhs[4]:NullMatrix);
    initialize_sx(nrhs>5?prhs[5]:NullMatrix);
}
...
void calculate_sy(double *sxc){
    ...
    for( j=0; j<M; ++j ){
        sy0[j] -= min_sy0;
        if(dither)
            sy[j] = sy0[j] + d*(2*drand48()-1);
        else
            sy[j] = sy0[j];
    }
}

/* Implement the vector quantizer for unit elements */
void calculate_sv1(){
    int i, v=(int)(*pv++), num_tentative, need, committed_ones=0, itn;
    double sy_max=-INFINITY, sy_min=INFINITY, *psy;
    double threshold, thresh_min, thresh_max;
    double endpoint_offset;
    if( v<-M || v>M )
        fatalError("v is out of the range [-M,M].");
    if( (v+M)%2 != 0 )
        if( M%2 == 0 )
            fatalError("v is not an even integer.");
        else
            fatalError("v is not an odd integer.");
    v = (v+M)/2;
    need = v;
...
void calculate_sv2(){
    ...
    if( v < -sum_dw || v > sum_dw )
        fatalError("v is out of the range [-sum(dw),sum(dw)]");
    if( (v+sum_dw)%2 != 0 )
        fatalError("v + sum(dw) must be an even integer.");
    ...
    if( selectElements((v+sum_dw)/2,0) )
